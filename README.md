...
| SonarQube Cloud static analysis | `.github/workflows/ci.yml`'s scan steps point at the original author's own SonarQube Cloud project (`SONAR_TOKEN`/`SONAR_PROJECT_KEY`/`SONAR_ORG`) by design — CI skips them if unset. Set your own to turn it back on; the quality gate's coverage condition will fail until coverage reporting is wired up |
